...
const AccusedTab = () => {
  ...
  console.log('Accused Data:', filteredComplaints.map(c => c.accused));

  // Compute race, gender, and age distributions for accused
  const { raceData, genderData, ageData } = useMemo(() => {
    const raceCounts: Record<string, number> = {};
    const genderCounts: Record<string, number> = {};
    const ageCounts: Record<string, number> = {};
    filteredComplaints.forEach(c => {
      if (c.accused && Array.isArray(c.accused)) {
        c.accused.forEach(accused => {
          const race = accused.race || 'Unknown';
          const gender = accused.gender || 'Unknown';
          const age = accused.age_group || 'Unknown';
  ...
  if (filteredComplaints.length === 0) {
    return (
      <div className="h-full flex items-center justify-center">
        <p className="text-portal-500">No accused data available</p>
      </div>
    );
  }
